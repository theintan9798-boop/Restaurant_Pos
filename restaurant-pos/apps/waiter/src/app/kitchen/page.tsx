...
export default function KitchenDisplayPage() {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-950 text-gray-100">
      <header className="bg-gray-900 px-6 py-3 flex items-center justify-between flex-shrink-0 border-b border-gray-800">
        ...
        <div className="flex items-center space-x-4 text-xs">
          <span className="text-gray-500">{user?.name || 'Kitchen'}</span>
          <button onClick={logout} className="text-gray-400 hover:text-white px-3 py-1 rounded border border-gray-600 hover:border-gray-400" title="Lock / Switch User">🔒 Lock</button>
        </div>
      </header>

      {/* Station filters */}
      <div className="flex bg-gray-900 border-b border-gray-800 px-4 space-x-1 flex-shrink-0">
        {STATIONS.map(station => (
          <button key={station.id} onClick={() => setActiveStation(station.id)}
            className={`px-4 py-2 text-xs font-medium rounded-t-md transition-colors ${activeStation === station.id ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800'}`}>
            {station.label}
          </button>
        ))}
      </div>
                ...
                <div key={orderId} className={`rounded-xl border ${c.border} ${c.bg} p-3 space-y-2`}>
                  ...
                  <div className="space-y-1.5">
                    {orderTickets.map(item => (
                      <div key={item.ticketId} className="flex items-center justify-between text-sm">
                        <div className="flex items-center space-x-2">
                          <span className="text-gray-200">{item.menuItemName}</span>
                          <span className="text-gray-500 text-xs">x{item.quantity}</span>
                          {item.notes && <span className="text-amber-400 text-[10px] italic">{item.notes}</span>}
                        </div>
